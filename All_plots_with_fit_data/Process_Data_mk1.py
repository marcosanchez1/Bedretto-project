...
#     
#     channel,unix_time
#     {0:{fitting_parameters:[A0,A1,...],charge:charge_0},1:{0:{fitting_parameters:[A0,A1,...],charge:charge_0}},unix_time_0
#         
...
def parse_wavecatcher_file(path):
    ...
    def finalize_channel(ev, ch_id):
        """Process waveform of a channel once the block ends."""
        if ch_id is None:
            return

        samples = ev["channels"][ch_id]["_samples"]
        if len(samples) == 0:
            return
        ...
        ev["channels"][ch_id]["fit_parameters"] = params
        ev["channels"][ch_id]["charge"] = charge

        # Remove samples to save memory and avoid storing them in the DataFrame
        del ev["channels"][ch_id]["_samples"]
        ...
        for line in f:
            ...
            if m_event:
                ...
                if current_event is not None:
                    finalize_channel(current_event, current_channel)
            ...
            if m_ch and current_event is not None:
                # Finalize previous channel before switching
                finalize_channel(current_event, current_channel)

                ch_id = int(m_ch.group(1))
                current_event["channels"][ch_id] = {
                    "_samples": [],          # temporary storage
                    "fit_parameters": None,
                    "charge": None
                }
...
def main():
    ...
    for filename in os.listdir(raw_folder):
        ...
        if filename.lower().endswith(".dat"):
            ...
            print(f'\nFile finished: {out_csv} in {tf - ti:.1f}s')
