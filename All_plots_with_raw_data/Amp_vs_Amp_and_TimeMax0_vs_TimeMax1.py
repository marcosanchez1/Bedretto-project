...
def main(df, rate, route_figure):
    ...
    h1 = ax1.hist2d(
            ...
            time1,
            bins=bins1,
            range=[[min(time0), max(time0)], [min(time1), max(time1)]],
            cmap='turbo'
            )
    ax1.set_title(f"TimeMax_CH0 vs TimeMax_CH1(samples={len(time0)};bins={bins1};rate={int(round(RATE,0))}Hz)")
    ax1.set_xlabel("TimeMax_CH0 (ns)")
    ax1.set_ylabel("TimeMax_CH1 (ns)")
    plt.colorbar(h1[3], label="Counts")
    ax1.grid(True)

    plt.tight_layout()
    plt.savefig(f"{route_figure}\\AmpvsAmp_Timevstime_2DHistograms.png")
    plt.show()
    plt.close()
