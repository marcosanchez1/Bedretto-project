...
def main(df, rate, route_figure):
    ...
    bins0 = int(round(N * np.sqrt(len(min0)),0))
    ax0.hist(
            min0,
            bins=bins0,
            alpha=0.7,
            range=[min(min0), max(min0)],
            label=f'bins={bins0};rate={int(round(RATE,0))}Hz; ;std={np.std(min0)}',
            histtype='step'
            )
    ax0.set_title(f"-Min values of noise - CH0 (samples={len(min0)})")
    ax0.set_xlabel("-Min values (ADC)")
    ax0.set_ylabel("Counts")
    ax0.legend()
    ax0.grid(True)

    bins1 = int(round(N * np.sqrt(len(min1)),0))
    ax1.hist(
            min1,
            bins=bins1,
            alpha=0.7,
            range=[min(min1), max(min1)],
            label=f'bins={bins1};rate={int(round(RATE,0))}Hz; ;std={np.std(min1)}',
            histtype='step'
            )
    ax1.set_title(f"-Min values of noise - CH1 (samples={len(min1)})")
    ax1.set_xlabel("-Min values (ADC)")
    ax1.set_ylabel("Counts")
    ax1.legend()
    ax1.grid(True)

    plt.tight_layout()
    plt.savefig(f"{route_figure}\\Min_Values_Noise_Histograms.png")
    plt.show()
    plt.close()
