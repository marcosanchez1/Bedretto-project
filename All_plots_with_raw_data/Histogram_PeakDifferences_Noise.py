'''
In this script I'll just perform two histograms of the max - min values of both channels noise parts aka the static line before
...
dt = 0.3125 #multiplying indices of samples by this gives it in ns
def main(df, rate, route_figure):
    ...
    for row in df['channels']:
        ...
        difference0.append(np.max(ch0_corr)-np.min(ch0_corr))

        ch1 = np.array(row[1])
        baseline = np.mean(ch1)   # or median
        ch1_corr = ch1 - baseline

        difference1.append(np.max(ch1_corr)-np.min(ch1_corr))

    fig, ax = plt.subplots(1, 2, figsize=(8, 8))
    ax0, ax1 = ax.flatten()
    N = 2
    
    bins0 = int(round(N * np.sqrt(len(difference0)),0))
    ax0.hist(
            difference0,
            bins=bins0,
            alpha=0.7,
            range=[min(difference0), max(difference0)],
            label=f'bins={bins0};rate={int(round(RATE,0))}Hz',
            histtype='step'
            )
    ax0.set_title(f"Peak differences(max-min) - CH0 (samples={len(difference0)})")
    ax0.set_xlabel("Peak difference (ADC)")
    ax0.set_ylabel("Counts")
    ax0.legend()
    ax0.grid(True)

    bins1 = int(round(N * np.sqrt(len(difference1)),0))
    ax1.hist(
            difference1,
            bins=bins1,
            alpha=0.7,
            range=[min(difference1), max(difference1)],
            label=f'bins={bins1};rate={int(round(RATE,0))}Hz',
            histtype='step'
            )
    ax1.set_title(f"Peak differences(max-min) - CH1 (samples={len(difference1)})")
    ax1.set_xlabel("Peak difference (ADC)")
    ax1.set_ylabel("Counts")
    ax1.legend()
    ax1.grid(True)

    plt.tight_layout()
    plt.savefig(f"{route_figure}\\Max-Min_Noise_Histograms.png")
    plt.show()
    plt.close()
