...
def main(df, rate, route_figure):
    ...
    max0 = [np.argmax(row[0])*dt for row in df['channels']]
    max1 = [np.argmax(row[1])*dt for row in df['channels']]
    ...
    ax0.set_title(f"Time position of peaks - CH0 (samples={len(max0)})")
    ax0.set_xlabel("Time position (ns)")
    ...
    ax1.set_title(f"Time position of peaks - CH1 (samples={len(max1)})")
    ax1.set_xlabel("Time position (ns)")
    ax1.set_ylabel("Counts")
    ax1.legend()
    ax1.grid(True)

    plt.tight_layout()
    plt.savefig(f"{route_figure}\\Position_Time_Max_Values_Noise_Histograms.png")
    plt.show()
    plt.close()
