...
def main():
    ...
    df_fit = pd.read_csv(f".\\Data\\Processed_data\\1Bar_2Chs\\Run_{Voltage}V_Run{run}_Data_{month}_{day}_2026_Ascii.csv")
    ...
    def update(frame):
        ...
        end = np.argmax(raw_ch0[frame]) + int(round(WINDOW/dt,0))
        ...
        end = np.argmax(raw_ch1[frame]) + int(round(WINDOW/dt,0))
        t_fit = np.arange(end)*dt
